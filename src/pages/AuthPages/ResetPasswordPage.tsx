        ...
        <div className="card">
          {done ? (
            <div className="text-center py-4">
              ...
              <Link to="/signin" className="inline-block mt-4 text-xs text-brand-600 hover:text-brand-700 font-medium">
                Back to sign in
              </Link>
            </div>
          ) : (
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button type="submit" disabled={loading} className="btn-primary w-full justify-center">
                  {loading ? <span className="w-3.5 h-3.5 border-2 border-white/40 border-t-white rounded-full animate-spin" /> : 'Send reset link'}
                </button>
              </form>

              <p className="mt-4 text-center text-xs text-gray-400">
                <Link to="/signin" className="text-brand-600 hover:text-brand-700 font-medium">Back to sign in</Link>
              </p>
            </>
          )}
        </div>
